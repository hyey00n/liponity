    ...
    <header ref={headerRef} className="border-b border-gray-200 sticky top-0 bg-white z-50">
      <div className="max-w-[1400px] mx-auto px-4 h-14 flex items-center gap-4">
        ...
        <div className="flex-1 flex justify-center relative">
            <div className="flex border border-gray-200 divide-x divide-gray-200 text-xs">
              ...
              >
                ...
                {travel.flightOverride
                  ? <span className="text-gray-500">${Number(travel.flightOverride.replace(/,/g, '')).toLocaleString()}</span>
                  : <span className="text-gray-400">~{fmt(estFlight)}</span>
                }
              </button>
            ...
            {/* Flying from dropdown */}
            {activeDropdown === 'city' && (
              <div className="absolute top-full left-0 z-50 bg-white border border-gray-200 shadow-lg p-4 mt-1 w-80">
                ...
                <div className="border-t border-gray-100 pt-3">
                  <div className="flex justify-between items-center mb-1">
                    <p className="text-xs text-gray-400">Flight (round-trip)</p>
                    {travel.flightOverride
                      ? <button onClick={() => travel.setFlightOverride('')} className="text-xs text-gray-300 hover:text-gray-700">reset</button>
                      : <span className="text-xs text-gray-300">~{fmt(estFlight)}</span>
                    }
                  </div>
                  <input
                    type="text" inputMode="numeric" placeholder="Your actual flight ($)"
                    value={travel.flightOverride} onChange={e => travel.setFlightOverride(e.target.value)}
                    className={`text-xs border px-2 py-1.5 w-full focus:outline-none transition-colors ${
                  ...
                  <a
                    ...
                    className="block text-xs text-gray-400 hover:text-gray-700 underline mt-1 mb-3"
                  >
                    Check Google Flights →
                  </a>
                  <button onClick={() => setActiveDropdown(null)} className="w-full text-xs bg-gray-900 text-white py-1.5 hover:bg-gray-700">Done</button>
                </div>
              </div>
            )}
